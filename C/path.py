...
class Tan():
    ...
    def Recognition_method(self, driver, identify_type, user_name, password, url, select_type_bny, keyword_bny,
                           issuer_name_bny, deal_name_bny, shelf_type_citimortgage, data_type_citimortgage, file_name,
                           issuer):  # 根据爬虫项目名称,识别调用对应的接口
        try:
            if "db" in identify_type:
                ...
                self.Popup_window_Bny(driver, user_name, password, select_type_bny, keyword_bny, issuer_name_bny,
                                      deal_name_bny)
                ...
                self.Popup_window_Citimortgage(user_name, password, shelf_type_citimortgage, data_type_citimortgage,
                                               file_name)
            else:
                print "待扩展的方法"
    ...
    def Popup_window_Db(self, url):  # Db爬虫弹窗接口
        ...
        url = 'https://tss.sfs.db.com/investpublic/servlet/web/ReportDownload?value=STMT,PDF,03,AS894,O,2018&page=history&OWASP_CSRFTOKEN='
        print "downloading with urllib"
        urllib.urlretrieve(url, "demo.pdf")
...
def Popup_window_FeddieMac(self, user_name, password):
    ...
    try:
        ...
        print "提交按钮定位失败"
    time.sleep(3)
    file_name = "bi180620_18DNA2.zip"
    driver.find_element_by_link_text(file_name).click()


if __name__ == '__main__':
    print "接口类型简介"
    print "第一种:方法名为Popup_window_Db,适用于无需登录就可以下载资源的页面,只需输入一个参数即可"
    print "第二种:方法名为Popup_window_Usbank,适用于登录过后才可以下载资源的页面,需输入 登陆页面的url,用户名的网页标签id值,密码的网页标签id值,用户名,密码,提交按钮的网页标签,下载页面的url  共7个参数"
    print "第三种:方法名为Popup_window_Bny,适用于bny爬虫,需输入 用户名,密码,共2个参数"

    spider = Tan()
    # user_name = "2632423486@qq.com"
    # password = "{lfufeQ3"
    spider.Popup_window_Db(
        "https://tss.sfs.db.com/investpublic/servlet/web/Web?document=viewhistory&link=&producttype=RMBS&issueid=AS894&displayScreen=R&FromBackUrl=true&year=2018&OWASP_CSRFTOKEN=AJ37-Z2XR-S29I-K0Q9-DIHM-BZFS-0BY1-2EY4")
